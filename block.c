...
struct ledger local_ledger = {NULL, 0, 0};
...
int sha256(uint8_t* input, size_t size, uint32_t* output)
{
    ...
    if(alloc && W == NULL) { // if realloc failed, we should free the original data
        memset(data, 0, sz); 
        free(data);
    }
    
    if(W != NULL) {
        memset(W, 0, 64 * 32); // clear the memory used for W
        free(W);
    }
...
struct block* init_block(size_t no_tr) // with a message, not binary data
{
    ...
    memset(b->sha, 0, 32);
    memset(b->header.merkle, 0, 32);
    b->header.nonce = 1;
    b->header.timestamp = time(NULL);
    b->header.version = MAKE_VERSION(1, 1, 0);
    b->header.difficulty = 0x1d00ffff; // to be modified later
    b->data.size = no_tr;
    b->data.t = (struct transaction*)(b + 1); // transactions are stored right after the block struct
    memset(b->data.t, 0, no_tr * sizeof(struct transaction));
    return b; 
}
...
void add_transaction_to_block(struct block* b, struct transaction* t)
{
    ...
    for(int i = 0;i < b->data.size;i++)
    {
        if(b->data.t[i].id == 0)
        {
            memcpy(&b->data.t[i], t, sizeof(struct transaction));
            return;
        }
    }
}
...
void print_block(struct block* b)
{
        ...
        print_hash(b->header.prev_sha, ' ');
        printf("\nMerkle Root: ");
        print_hash(b->header.merkle, ' ');
        printf("\nNonce: %u\n", b->header.nonce);
        printf("Timestamp: %lu\n", b->header.timestamp);
        printf("Difficulty: %u\n", b->header.difficulty);
